...
// License: BSD
//
// This is a port-local copy of LiteX's liblitesdcard/sdcard.c, kept in sync
// with upstream because:
//   * MicroPython's link rules (extmod.mk) pull in lib/oofatfs/ff.c, while
//     the LiteX-bundled library uses libfatfs. We disable the FatFs glue
//     in this file and instead route through machine_sdcard.c.
//   * machine_sdcard.c needs sdcard_numblocks() to answer
//     ioctl(BLOCK_COUNT) — upstream doesn't expose that, so we add it
//     here (computed from the CSD on init).
//   * sdcard_write() takes a `const uint8_t *` to match our header and
//     callers (extmod block protocol passes const buffers).
//
// When updating, diff against
// litex/litex/soc/software/liblitesdcard/sdcard.c and re-apply just those
// three deltas.
...
/*
 * Number of 512-byte sectors decoded from the most recent CMD9 (SEND_CSD)
 * response. machine_sdcard.c reads this via sdcard_numblocks() to answer
 * ioctl(BLOCK_COUNT) without re-issuing CMD9.
 *
 * Layout assumes CSD v2.0 (SDHC/SDXC). C_SIZE is 22 bits at CSD bits 69:48,
 * which lands in the response register (with this hardware's MSB-first
 * layout starting at r[0]) as:
 *   bits[15:0]  in r[1] high half  -> (r[1] >> 16) & 0xffff
 *   bits[21:16] in r[2] low byte   -> (r[2] & 0xff) << 16
 * Capacity (in 512 B sectors) = (C_SIZE + 1) * 1024.
 */
static uint32_t sdcard_nsects;

void sdcard_decode_csd(void) {
	uint32_t r[SD_CMD_RESPONSE_SIZE/4];
	csr_rd_buf_uint32(CSR_SDCARD_CORE_CMD_RESPONSE_ADDR,
			  r, SD_CMD_RESPONSE_SIZE/4);
	uint32_t c_size = (r[1] >> 16) | ((r[2] & 0xff) << 16);
	sdcard_nsects = (c_size + 1) * 1024;
#ifdef SDCARD_DEBUG
	...
	printf(
		...
		"Device size: %" PRIu32 " sectors (%" PRIu32 " MiB)\n",
		...
		sdcard_nsects, sdcard_nsects / 2048
	);
#endif
}
...
/*-----------------------------------------------------------------------*/
/*
 * Only compiled when this is a libfatfs build (libliteSDcard linked
 * against LiteX's bundled libfatfs). MicroPython uses lib/oofatfs/ff.c
 * (gated by FFCONF_H / MICROPY_VFS_FAT) and routes through
 * machine_sdcard.c instead, so we skip these symbols there to avoid
 * dragging in DSTATUS / BYTE / DRESULT typedefs that aren't defined.
 */
#if !defined(MICROPY_VFS_FAT) && !defined(FFCONF_H)
...
#endif /* !MICROPY_VFS_FAT && !FFCONF_H */
